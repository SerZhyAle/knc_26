...
Private Sub lpl(x1 As Integer, y1 As Integer, x2 As Integer, y2 As Integer, xy As Integer, atx As Integer, aty As Integer)
 ...
 '
'  Images(zz + z * ww).Visible = True

    'Line (x1, y1)-(x2, y2), RGB(p_(1), p_(2), p_(3)), BF
   'Circle ((x1 + x2) / 2, (y1 + y2) / 2), ww * 15, RGB(r, g, B)
  ' Paint ((x1 + x2) / 2, (y1 + y2) / 2), RGB(r, g, B), RGB(r, g, B)
...
End If
If lost = 1 Then
 sms = MsgBox("ИГРА ПРОИГРАНА!", vbOKOnly, "Помощник врага нашел вас!")
 z = BL_end
 GGOO = 2
 GoGo = 1
Else
...
End If
If lost = 1 Then
 sms = MsgBox("ИГРА ПРОИГРАНА!", vbOKOnly, "Враг нашел вас!")
 GGOO = 2
 GoGo = 1
Else
 gh = Int(Rnd * 2) + 1
